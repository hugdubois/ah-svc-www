...
func (s *wwwServer) ServicesStatus(ctx context.Context, req *pb.EmptyMessage) (*pb.ServicesStatusList, error) {
	...
	fns := []func(*wwwServer, context.Context, context.Context) *pb.ServiceStatus{
		svcCurrentStatus,
	...
	wg.Add(len(fns))
	for _, fn := range fns {
		go func(f func(*wwwServer, context.Context, context.Context) *pb.ServiceStatus) {
			defer wg.Done()
