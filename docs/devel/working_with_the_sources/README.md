...
ah-svc-www needs some requirements :
...
mkdir -p $GOPATH/src/github.com/hugdubois
cd $GOPATH/src/github.com/hugdubois
git clone https://github.com/hugdubois/ah-svc-www.git
cd ah-svc-www
...
- `make dep-update-hugdubois [individual svc name without ah-svc- prefix|default all]` - Executes the `dep ensure -update github.com/hugdubois/ah-svc-[individual svc name without ah-svc- prefix|default all]`
...
If ah-svc-www repository is private and you use [Gogs](https://gogs.io/) has remote server.
...
```bash
rm $GOPATH/bin/ah-svc-www
rm -rf $GOPATH/src/github.com/hugdubois
```
...
- To add a Hugdubois subservice as dependency see [this](../add_sub_service/README.md)
